    ...
    *,
    ...
    tx = [
        ...
        *data_cfg.data_transforms.inputs,
        transforms.Normalize(norm_stats, normalization_type=data_cfg.action_proprio_normalization_type),
...
) -> up.DataLoader[tuple[Observation, _model.Actions]]:
...
class RLDSDataLoader:
    ...
    def _assert_divisible(self, batch):
        ...
        if isinstance(self._sharding, jax.sharding.NamedSharding):
            ...
            data_axis_size = mesh.shape.get("batch", None)  # or use your DATA_AXIS constant
            if data_axis_size is None:
                raise ValueError("Sharding has no 'batch' axis")
