...
def get_dataset_statistics(
    ...
    Args:
        ...
        save_dir: Directory to save/load statistics from
        action_key: Key for actions in the trajectory dict
        state_key: Key for state in the observation dict
        action_dim: If provided, 3D actions [T, H, D] will be reshaped to [-1, action_dim]
    """
    ...
    for traj in tqdm(dataset.iterator(), total=cardinality):
        action_data = traj[action_key]
        # Handle 3D actions [T, H, D] by reshaping to [-1, D]
        if len(action_data.shape) == 3:
            # action_data shape: [T, action_horizon, action_dim]
            T, H, D = action_data.shape
            action_data = action_data.reshape(-1, D)  # [T * H, D]
        actions.append(action_data)
        proprio_data = traj["observation"][state_key]
        proprios.append(proprio_data)
        if state_dim is None:
            state_dim = proprio_data.shape[-1]
        num_transitions += action_data.shape[0]
        num_trajectories += 1

    # Handle empty shard case (can happen when dataset has fewer trajectories than hosts)
    # We create empty arrays with correct shape so multi-host aggregation still works
    if len(actions) == 0:
        if action_dim is None:
            raise ValueError(
                "Dataset shard is empty and action_dim not provided. "
                "Cannot determine action dimensions for multi-host aggregation. "
                "Pass action_dim parameter to get_dataset_statistics()."
            )
        logging.warning(
            f"This host's dataset shard is empty (cardinality={cardinality}). "
            f"This can happen when the dataset has fewer trajectories than hosts. "
            f"Stats will be aggregated from other hosts."
        )
        # Create empty arrays with correct dimensions for aggregation
        actions = np.zeros((0, action_dim), dtype=np.float32)
        proprios = np.zeros((0, action_dim), dtype=np.float32)  # Use action_dim as fallback
    ...
    # Check if local shard is empty (can happen when dataset has fewer trajectories than hosts)
    local_shard_empty = actions.shape[0] == 0
    a_dim = actions.shape[1]
    s_dim = proprios.shape[1] if has_state else 0
    ...
    # For empty shards, use +inf/-inf so they don't affect global min/max
    if local_shard_empty:
        a_min = np.full(a_dim, np.inf, dtype=np.float64)
        a_max = np.full(a_dim, -np.inf, dtype=np.float64)
    else:
    ...
    if has_state:
        if local_shard_empty:
            s_min = np.full(s_dim, np.inf, dtype=np.float64)
            s_max = np.full(s_dim, -np.inf, dtype=np.float64)
        else:
